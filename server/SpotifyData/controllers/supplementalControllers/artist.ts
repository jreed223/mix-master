...
import { Album, AlbumList, Artist, Track } from '../../../types';
type FetchResponse = Response;  //Fetch API Response

export async function fetchArtist(accessToken:string, id: string): Promise<FetchResponse> {
...
export const artist = (req: expressRequest, res: expressResponse)=>{
    const accessToken = req.cookies.access_token? req.cookies.access_token:res.locals.access_token
    const id= req.body.id
    //console.log(req.cookies)
    if(accessToken){
        fetchArtist(accessToken, id).then(async (response:FetchResponse)=>{
            if(response.ok){
                console.log("OK response from spotify-data/artist")
