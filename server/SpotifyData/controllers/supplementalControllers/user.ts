...
import { Album, AlbumList, Track, UserProfile } from '../../../types';
type FetchResponse = Response;  //Fetch API Response

export async function fetchUser(accessToken:string, id: string): Promise<FetchResponse> {
...
export const user = (req: expressRequest, res: expressResponse)=>{
    const accessToken = req.cookies.access_token? req.cookies.access_token:res.locals.access_token
    const id= req.body.id
    //console.log(req.cookies)
    if(accessToken){
        fetchUser(accessToken, id).then(async (response:FetchResponse)=>{
            if(response.ok){
