...
import { Album, AlbumList, SearchResults, Track, Tracklist } from '../../../types';
type FetchResponse = Response;  //Fetch API Response

export async function fetchUsersPlaylists(accessToken:string, id: string): Promise<FetchResponse> {
...
export const usersPlaylists = (req: expressRequest, res: expressResponse)=>{
    const accessToken = req.cookies.access_token? req.cookies.access_token:res.locals.access_token
    const id= req.body.id
    //console.log(req.cookies)
    if(accessToken){
        fetchUsersPlaylists(accessToken, id).then(async (response:FetchResponse)=>{
            if(response.ok){
                console.log("OK response from spotify-data/album")
                const playlistsObject:SearchResults['playlists'] = await response.json();
                
            
                res.send(playlistsObject)
            }else{
