...
import { LikedTracks, PlaylistItem, SearchResults, Tracklist } from '../../../types';
// type FetchResponse = Response;  //Fetch API Response




export const nextPlaylists = async (req: expressRequest, res: expressResponse)=>{
    ...
    if(accessToken && nextLink){
      const nextPlaylists:SearchResults['playlists'] = await  fetch(nextLink,{
        ...
        if(nextPlaylists){
            const playlists = nextPlaylists.items;
