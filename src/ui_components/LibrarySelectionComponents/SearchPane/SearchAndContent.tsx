        ...
        <form style={{ height: "100%", width: "100%", minWidth: isMobile ? "calc(100vw  - 115px)" : "calc(50vw - 115px)" }}>
            ...
            <div className={"search-bar2"} style={{ height: "100%", width: "100%", overflowX: 'clip', position: 'relative' }}>
                <div style={isMobile ? { width: "100%", height: "100%", display: "flex", flexDirection: "column" } : { display: "flex", flexDirection: "column", width: "75%", minWidth: "50vw", height: "100%", margin: "auto" }}>
                    <div style={{ alignContent: "center", display: "flex", flexDirection: isMobile ? "column" : "row", margin: "auto 10px", height: isMobile?"20%":"10%" }}>
                        <div style={{ width: isMobile?"100%":"45%", display: "flex", justifyContent: "center", alignItems:"center" }}>
                            <input ref={searchInputRef} style={{ minWidth: "90%", maxHeight: "40px", color: "#878787", fontSize: "1.5em", borderRadius: "25px", paddingLeft: "15px", backgroundColor: "rgb(33 33 33)", border: "none", margin: "10px" }} type="search" placeholder="Search..." value={searchQuery} onKeyDown={(e) => { onEnter(e) }} onChange={(e) => { e.preventDefault(); setSearchQuery(e.target.value) }}></input>
                        </div>

                        <div style={{display:"flex", flex:1, justifyItems:"center", alignItems:"center"}}>
                            <div style={{ transition: "1s", display: "inline-flex", alignItems: "center", maxWidth: searchResults ? "100%" : "0%", width:searchResults?"100%":"0%", gap: searchResults ? "4%" : "0%", justifyContent: "center", overflow: "hidden" }}>
                            ...
                            <div style={{ cursor: "pointer", flex: "1 1 auto ", margin:"auto 5px", width:(stagingState==="open"&&searchResults)||(isMobile&&searchResults)?"33px":"177px", transition: "1s",display:"flex", justifyContent:"center", alignItems:"center" }}>
                            <img src="/spotify/Primary_Logo_Green_RGB.svg" style={{maxWidth: "33px"}} alt="spotify logo"/><p style={{ whiteSpace:"nowrap", fontSize:"1em", overflow:"hidden", margin:0, opacity:(stagingState==="open"&&searchResults)||(isMobile&&searchResults)?0:1, transition: '1s'}}>Open Spotify</p>
